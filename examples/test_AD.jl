...
simshow(abs2.(Array(res.minimizer[:, :, iangle])) .* 1, cmap=:turbo, γ=1, set_one=false)
...
#=╠═╡
[simshow(view(fwdd, ix, :, :), γ=1, set_one=false, cmap=:turbo) simshow(view(fwdd, ix, :, :) .> thresh, γ=1, set_one=true, cmap=:turbo)]
  ╠═╡ =#

# ╔═╡ c924eab9-0bd6-4dfc-a41e-6d82a1cc22ac
abs2.(res.minimizer)
...
function plot_histogram(img, object_printed, thresholds, chosen_threshold)
	histogram(object_printed[img .== 0], bins=(0.0:0.01:1), xlim=(0.0, 1.0), label="dose distribution void", ylabel="voxel count", xlabel="normalized intensity",  ylim=(1, 1000000),  yscale=:log10, linewidth=1, legend=:topleft, size=(1000, 700))
...
# ╔═╡ 49bb6ce4-550d-43db-be28-5ac285d96037
#=╠═╡
